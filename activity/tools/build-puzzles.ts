...
function main(): void {
  ...
  // `requiredClears` is rebuilt, not carried across. It used to be carried,
  // because a separate pass wrote it and a rebuild that omitted it would
  // silently un-enforce every puzzle that had one. `buildPuzzle` now derives it
  // from the answer it has just replayed, so the rebuild supplies it — and
  // carrying the old value over the new one would now be the bug: a maker who
  // edits their answer would keep a rule derived from the answer they replaced,
  // which is exactly the "demands a clear its own solution never makes" state
  // that `withoutUnmeetableClears` exists to catch at load.
  const prompts = puzzles.map(({ solution: _s, source: _src, ...prompt }) => prompt);
  const enforcing = prompts.filter((prompt) => prompt.requiredClears?.length).length;
  console.log(`${enforcing} of ${prompts.length} puzzles carry a clear requirement`);
